...
public static class FilterExpressionBuilder
{
    ...
    private static Expression BuildOperationExpression(Expression left, FilterOperator filterOperator, object? rightValue)
    {
        ...
        if (filterOperator == FilterOperator.In || filterOperator == FilterOperator.NotIn)
        {
            ...
            if (TimeDistinctHelper.IsTimeOfDayType(left.Type))
            {
                return BuildInExpressionForTimeOfDay(left, rightValue, filterOperator == FilterOperator.NotIn);
            }
    ...
    private static Expression BuildInExpressionForTimeOfDay(Expression left, object? value, bool invert)
    {
        if (value is not System.Collections.IEnumerable enumerable || value is string)
        {
            throw new ArgumentException("Value must be an IEnumerable for In/NotIn operations.");
        ...
        {
            var equal = Expression.Call(
                typeof(TimeDistinctHelper),
                nameof(TimeDistinctHelper.MatchesTimeOfDay),
                Type.EmptyTypes,
                leftObject,
                Expression.Constant(val, typeof(object)));
