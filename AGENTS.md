...
CI (`.github/workflows/dotnet.yml`): single `build` job (restore, build, **unit tests only**, pack). UI integration suites (Playwright, FlaUI, Appium) are **excluded** from GitHub Actions — run locally per `tests/UI.ContractTesting.md`.
...
| **Test** | Yes | Excludes `UIContracts.*` and `PlaywrightTests` (no UI integration on GHA/act). |
| **Pack / NuGet push** | Pack on every run; push only on `v*` tags | Skipped on `pull_request` for NuGet push. |
| **UI integration** (Playwright / FlaUI / Appium) | No | Run locally only. |
